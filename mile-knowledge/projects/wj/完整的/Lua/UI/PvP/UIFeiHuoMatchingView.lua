-- ---------------------------------------------------------------------------------
-- Author: 陈计
-- Name: UIFeiHuoMatchingView
-- Date: 2023-08-02 19:51:59
-- Desc: 飞火论锋报名
-- Prefab: PanelFeiHuoMatching
-- ---------------------------------------------------------------------------------

local UIFeiHuoMatchingView = class("UIFeiHuoMatchingView")

local nPersonalScoreIndex  = BF_MAP_ROLE_INFO_TYPE.MATCH_LEVEL
local m_tSumPersonalInfo   = {}

---_LuaBindList 在这里对绑定的组件进行无意义自赋值，从而方便查询以及ide进行智能提示
function UIFeiHuoMatchingView:_LuaBindList()
    ...
    self.ImgBg                = self.ImgBg --- 背景图 - 飞火论锋
    self.ImgBg2               = self.ImgBg2 --- 背景图 - 李渡鬼域

    self.ImgBattleName        = self.ImgBattleName --- 左上角标题图 - 飞火论锋
    self.ImgBattleName2       = self.ImgBattleName2 --- 左上角标题图 - 李渡鬼域
end

function UIFeiHuoMatchingView:OnEnter(dwNpcID, dwMapID, dwBattlefieldMapType)
    -- 地图ID
...
function UIFeiHuoMatchingView:OnExit()
    self.bInit = false
    self:UnRegEvent()
end

function UIFeiHuoMatchingView:BindUIEvent()
    ...
    UIHelper.BindUIEvent(self.BtnPersonal, EventType.OnClick, function()
        if not PakDownloadMgr.UserCheckDownloadMapRes(self.dwMapID, nil, nil, nil, "飞火论锋") then
            return
        end
...
function UIFeiHuoMatchingView:RegEvent()
    --个人战绩
...
function UIFeiHuoMatchingView:UnRegEvent()
...
function UIFeiHuoMatchingView:UpdateInfo()
    self:UpdateTextInfo()
    self:InitBlackStateTime()
    self:UpdateBtnState()

    -- 不同玩法显示不同的背景图
    -- hack: 由于这里 SetSpriteFrame 修改背景图时，图片会不符合预期，暂时先用两个不同节点控制隐藏显示来实现
    UIHelper.SetVisible(self.ImgBg, self.dwBattlefieldMapType == BATTLEFIELD_MAP_TYPE.FBBATTLE)
    UIHelper.SetVisible(self.ImgBg2, self.dwBattlefieldMapType == BATTLEFIELD_MAP_TYPE.ZOMBIEBATTLE)

    UIHelper.SetVisible(self.ImgBattleName, self.dwBattlefieldMapType == BATTLEFIELD_MAP_TYPE.FBBATTLE)
    UIHelper.SetVisible(self.ImgBattleName2, self.dwBattlefieldMapType == BATTLEFIELD_MAP_TYPE.ZOMBIEBATTLE)
...
function UIFeiHuoMatchingView:UpdateTextInfo()
    local szTitle
...
--更新匹配/惩罚时间
function UIFeiHuoMatchingView:OnUpdateTime()
...
--匹配时间
function UIFeiHuoMatchingView:InitMatchTime(dwMapID)
...
--界面上更新匹配时间
function UIFeiHuoMatchingView:UpdateMatchTime()
...
--惩罚时间
function UIFeiHuoMatchingView:InitBlackStateTime()
...
--惩罚时间更新
function UIFeiHuoMatchingView:UpdateBlackStateTime()
...
function UIFeiHuoMatchingView:UpdateBtnState()
    local bInQueue         = BattleFieldQueueData.IsInBattleFieldQueue(self.dwMapID)
...
function UIFeiHuoMatchingView:OnSyncBFRoleDate(dwPlayerID, dwMapID, bUpdate, eType)
...
function UIFeiHuoMatchingView:UpdateBF_LYJ(nHaveCount, nMaxCount)
    local nAcquireNum = nMaxCount - nHaveCount
    if nAcquireNum < 0 then
        nAcquireNum = 0
    end

    self.coinScript = self.coinScript or UIHelper.AddPrefab(PREFAB_ID.WidgetCoin, self.WidgetPVPMoney, CurrencyType.LeYouBi)
    -- self.coinScript:SetCurrencyCount(nHaveCount)

    UIHelper.SetString(self.LabelExtraNum, nAcquireNum)
end

return UIFeiHuoMatchingView