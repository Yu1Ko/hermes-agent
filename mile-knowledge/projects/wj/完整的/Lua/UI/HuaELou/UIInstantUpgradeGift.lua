-- ---------------------------------------------------------------------------------
-- Author: Unknown
-- Name: UIInstantUpgradeGift
-- Date: 2023-05-22 14:53:06
-- Desc: 简单按钮活动
-- ---------------------------------------------------------------------------------

local UIInstantUpgradeGift = class("UIInstantUpgradeGift")

function UIInstantUpgradeGift:OnEnter(dwOperatActID, nID)
...
function UIInstantUpgradeGift:OnExit()
    self.bInit = false
    self:UnRegEvent()
end

function UIInstantUpgradeGift:BindUIEvent()

end

function UIInstantUpgradeGift:RegEvent()
...
function UIInstantUpgradeGift:UnRegEvent()
...
-- ----------------------------------------------------------
function UIInstantUpgradeGift:UpdateBtnPos(tActivity)
...
function UIInstantUpgradeGift:GetBtnInfo(k, tActivity)
...
function UIInstantUpgradeGift:UpdateInfo()
...
function UIInstantUpgradeGift:SyncExtPoint(tLine)
...
function UIInstantUpgradeGift:RemoteCallBatchCheck(tLine)
...
function UIInstantUpgradeGift:On_Recharge_CheckWelfare_CallBack(nLimit, nReward, nMoney, bActive, bIsBespoke, dwID, tCustom, szCustom)
    self.tReward = {
		[1] = {nLimit, nReward},
	}
...
function UIInstantUpgradeGift:DelegateInitMainPage2SpecificActivities(tLine)
...
function UIInstantUpgradeGift:UpdataSpecialInfo()
...
function UIInstantUpgradeGift:UpdateTimeInfo(tLine)
...
function UIInstantUpgradeGift:UpdataRewardItem()
...
function UIInstantUpgradeGift:UpdataRewardItem_60()
    local tReward = HuaELouData.GetShowReward(self.nID)
    if tReward and not table_is_empty(tReward) then
...
function UIInstantUpgradeGift:UpdataRewardSpecialItem()
    if self.dwOperatActID ~= 193 then
        return
    end
...
function UIInstantUpgradeGift:UpdataRewardItem_Layout()
...
function UIInstantUpgradeGift:UpdataRewardItem_Scrollview()
...
function UIInstantUpgradeGift:UpdataItemScript(itemScript, tReward, k)
    if itemScript then
...
function UIInstantUpgradeGift:CheckAndProcessHandleOfBuy()
    if self.tbBtnShopBuy or self.dwOperatActID == OPERACT_ID.NEWYEAR_GIFT then
        local tbBtnList = self.tbBtnShopBuy or self.tbPublicBtn
        for v, tBtn in ipairs(tbBtnList) do
...
function UIInstantUpgradeGift:GetTargetListBtn(szLink)
    if not self.tbPublicBtn then
        return
    end
