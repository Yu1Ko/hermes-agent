local UIPanelActivityStore = class("UIPanelActivityStore")
...
local function GetBuyCountInfo(item, nMaxCount, tPrice)
    ...
    if (item.bCanStack or item.nGenre == ITEM_GENRE.BOOK) and item.nMaxStackNum > 1 then
        ...
        if not item.bCanStack and item.nGenre == ITEM_GENRE.BOOK then
            --??规则不明，照搬原先的
...
local function AutomaticSell(nNpcID, nShopID, player)
    local tPackageIndex = {
...
local function CanMutiBuy(goods, aShopInfo)
...
local function GoldSilverAndCopperToMoney(nGold, nSilver, nCopper)
    local nMoney = 0
    if nGold > 0 then
...
-------------------------------筛选数据-----------------------------------------
local function SplitStringS(szFullString, szSeparator)
...
-- 获取筛选项文字
local function GetSelectorText(szSelectorName, oValue)
    -- 默认标题
    if oValue == nil then
...
-- 人工修正部分商品的筛选
local function FixGoodsSelector(goods, szSelectorName)
...
-- 生成商品筛选标签
local function GenerateGoodsSelector(goods, szSelectorName)
    local iteminfo = ShopData.GetItemInfoByGoods(goods)
...
-----------------------------事件处理--------------------------------------
function UIPanelActivityStore:OnEnter(...)
    if not self.bInit then
        self:RegEvent()
        self:BindUIEvent()
        self.bInit = true

        self:InitItemTipScript()

        UIHelper.ToggleGroupRemoveAllToggle(self.ToggleNavigation)
        for _, node in ipairs(self.tbNavigationToggles) do
            UIHelper.ToggleGroupAddToggle(self.ToggleNavigation, node)
        end
    end
    ...
    self.nClassID = 1
    self.nFullScreenType = args[3] or 1
    ...
    self.szScenePath = Const.SHOP_SCENE
    
    -- 初始化模型管理器
    if not self.modelManager then
        self.modelManager = CommonStoreModelManager.CreateInstance(CommonStoreModelManager)
    end
    self.modelManager:Init(self.MiniScene, self.TouchContainer, Const.SHOP_SCENE, { 2, 0, 0 })

    self:UpdateSkin()
end

function UIPanelActivityStore:OnExit()
...
function UIPanelActivityStore:BindUIEvent()
...
function UIPanelActivityStore:RegEvent()
...
function UIPanelActivityStore:OnFrameBreathe()
...
function UIPanelActivityStore:TableView_SetupArrow(tableView, arrowParent)
...
function UIPanelActivityStore:UpdateShopItemTableView()
...
function UIPanelActivityStore:OnShopUpdateItem(nShopID, dwItemIndex, bAdd, nItemTemplateIndex)
    ...
    if self.bEquipSet and self.dwDefaultTabType then
        ...
        if tOtherShopInfo and tOtherShopInfo.dwTabType == self.dwDefaultTabType and tOtherShopInfo.dwIndex == self.dwDefaultIndex then
            ...
            if tbGoods.tKungfuMatchMap and not tbGoods.tKungfuMatchMap[dwMainKungfuID] then
                -- 允许再跳一次当前心法
...
function UIPanelActivityStore:GetGoods(dwItemIndex)
    local targetGoods
    local nGoodsIndex
...
function UIPanelActivityStore:OnSelectedGoodsChanged(nIdx, tbGoods)
...
function UIPanelActivityStore:OnTableViewUpdate(striptRow, nIndex)
    if not striptRow then
        return
    end

    striptRow:OnEnter(PREFAB_ID.WidgetStoreItem, self.ToggleGroupBag, self.nColumnCount)
    for i = 1, self.nColumnCount do
        ...
        if bVisible then
            ...
            if self.nCurGoodsIndex ~= nIdx and bSelected then
                -- 当前组件显示的不是被选中项，但是处于选中状态
                UIHelper.SetToggleGroupSelectedToggle(self.ToggleGroupBag, self.ToggleDefaultGoods)
            elseif self.nCurGoodsIndex == nIdx and not bSelected then
                -- 当前组件显示的是被选中项，但是未处于选中状态
                UIHelper.SetToggleGroupSelectedToggle(self.ToggleGroupBag, scriptCell.ToggleSelect)
            end
...
function UIPanelActivityStore:OpenShop(nShopID)
...
function UIPanelActivityStore:InitShopInfo()
    local npcName = ""
...
function UIPanelActivityStore:InitGlobalShopInfo()
...
function UIPanelActivityStore:UpdateShopLayout()
    --UIHelper.SetVisible(self.ImgItemDisplay, self.bShowDetailImage) -- 暂时屏蔽
    UIHelper.SetVisible(self.ImgItemDisplay, false) -- 暂时屏蔽

    local tTableViewNodes = { self.WidgetContentFour_WithTab, self.WidgetContentThree_WithTab, self.WidgetContentTwo_WithTab,
                              self.WidgetContentFive, self.WidgetContentThree, self.WidgetContentTwo }
    local tTargetTableViewNode = {
        [true] = {
            [4] = self.WidgetContentFour_WithTab,
            [3] = self.WidgetContentThree_WithTab,
            [2] = self.WidgetContentTwo_WithTab,
        },
        [false] = {
            [5] = self.WidgetContentFive,
            [3] = self.WidgetContentThree,
            [2] = self.WidgetContentTwo,
        }
    }

    for _, node in ipairs(tTableViewNodes) do
        UIHelper.SetActiveAndCache(self, node, false)
    end

    local bShowTab = true
    local nCol = 2
    local tOldTableView = self.TableView

    local node = tTargetTableViewNode[bShowTab][nCol]
    if node then
        local script = UIHelper.GetBindScript(node)
        self.TableView = script.TableView
        self.WidgetArrowParent = script.WidgetArrowParent
        self.nColumnCount = nCol
        ...
        if tOldTableView ~= self.TableView then
            self.TableView:setDataSource()
            self.TableView:setDataLen(0)
            self.TableView:reloadData() -- 切换tableview时清空
        end
    end
end

function UIPanelActivityStore:UpdateShopGroup(nNpcID, tShopGroup)
    self.nNpcID = nNpcID
    self.bGroup = true
...
function UIPanelActivityStore:UpdateShopClass(bNeedChangeShopID)
    ...
    for nClassID, tClass in ipairs(self.tShopGroup) do
        local szClassName = tClass.szName or tClass.szClassName -- 增加分类
    ...
    UIHelper.ScrollViewDoLayoutAndToTop(self.ScrollViewClassList)
end

function UIPanelActivityStore:RefreshWidgetBtns()
...
function UIPanelActivityStore:OnHideAllHoverTips()
    self.bBtnScreenVisable = false
    UIHelper.SetVisible(self.WidgetAnchorCategoryTips, self.bBtnScreenVisable)
    self.bWidgetAnchorSellTipsVisable = false
    UIHelper.SetVisible(self.WidgetAnchorSellTips, self.bWidgetAnchorSellTipsVisable)
...
function UIPanelActivityStore:ClearCurrency()
...
function UIPanelActivityStore:RemarkCurrency(dwItemIndex)
...
function UIPanelActivityStore:RemarkCostItem(dwItemIndex)
    local tOtherInfo = GetShopItemBuyOtherInfo(self.nShopID, dwItemIndex)
    if not tOtherInfo then
        return
    end
...
function UIPanelActivityStore:ShowBuyGoodsConfirm(nBuyCount)
...
function UIPanelActivityStore:SwitchShop(nShopID)
    self:ClearGoods()
    self:ClearCells()
    self:OnHideAllHoverTips()
    ...
    if type(nShopID) == 'number' then
        ...
        for nClassID, tClass in ipairs(self.tShopGroup) do
            if nClassID == self.nClassID then
                for _, tShop in ipairs(tClass) do
                    if tShop.bShow and self.nShopID == tShop.nShopID then
                        self.bShowDetailImage = tShop.bShowPic
                        self:UpdateShopLayout()
                    end
                end
            end
        end
...
function UIPanelActivityStore:ClearGoods()
...
function UIPanelActivityStore:AddGoods(goods, bSoldList)
    goods.tSelector = {}
...
function UIPanelActivityStore:BuyGoods(nBuyCount)
...
function UIPanelActivityStore:AutoBuyGoods()
...
function UIPanelActivityStore:QueryAllData()
    local tTables = {}
    local tIndexTable = {}
    for _, goods in ipairs(self.aGoods) do
        if type(goods.nShopID) == 'number' then
...
function UIPanelActivityStore:ClearCells()
...
function UIPanelActivityStore:UpdateCell()
    self:ClearCells()
    self:QueryAllData()
end

function UIPanelActivityStore:UpdateBuyBackCell()
    self:ClearCells()
end

function UIPanelActivityStore:InitItemTipScript()
    self.scriptItemTip = self.scriptItemTip or UIHelper.AddPrefab(PREFAB_ID.WidgetItemTip_PercentAnchor, self.WidgetCard)
    self.scriptQuantityController = self.scriptQuantityController or UIHelper.AddPrefab(PREFAB_ID.WidgetItemTipQuantityController_PublicStore, self.scriptItemTip.LayoutContentAll)
    self.WidgetItemTipQuantityController = self.scriptQuantityController._rootNode
    self.scriptItemTip:HideAllBackground()
    self.scriptItemTip:BindItemShare(self.BtnItemShare)

    UIHelper.SetVisible(self.WidgetCard, false)
    UIHelper.SetOpacity(self.WidgetCard, 0)
    Timer.Add(self, 0.2, function()
        UIHelper.SetOpacity(self.WidgetCard, 255) -- 防止闪烁
    end)
end

function UIPanelActivityStore:UpdateSelectedGoodsDetails()
    ...
    if self.nCurGoodsIndex and self.nCurGoodsIndex > 0 then
        ...
        if bItem and bCanShow then
            local bFirstInitItp = self.scriptItemTip == nil
            ...
            if self.modelManager then
                self.modelManager:UpdateModelInfo(goods.nItemType, goods.nItemIndex) -- 使用模型管理器更新模型信息
            end
            self.scriptItemTip:HideSource()
            ...
            if self.scriptQuantityController then
                ...
                self.scriptQuantityController:SetPreviewBtn(false) -- 目前支持模型 不显示试穿
                self.scriptQuantityController:SetWaitSell(bWaitSell, "敬请期待")

                -- 活动商店专属内容
                local szImgPath = Table_GetItemLargeIconPathByItemUiId(hItem.nUiId)
                UIHelper.SetTexture(self.ImgItemDisplay, szImgPath)
                local tTipInfo = self.scriptItemTip:GetSpecificContent()
                UIHelper.SetLabel(self.LabelItemName, tTipInfo.szName) -- 商品标题
                local bHasLabelCollected = tTipInfo.szCollected == "已收集" or tTipInfo.szCollected == "未收集"
                UIHelper.SetVisible(self.WidgetItemCollect, bHasLabelCollected) -- 是否收集标签
                UIHelper.SetLabel(self.LabelCollect, tTipInfo.szCollected) -- 是否已收集
                UIHelper.LayoutDoLayout(self.LayoutDetailTitle)

                if not bFirstInitItp then
                    self:RefreshItemTipsScrollViewHeight()
                end
            end
        end
...
function UIPanelActivityStore:ClearItemTips()
     UIHelper.SetVisible(self.WidgetCard, false)
     if self.scriptItemTip then self.scriptItemTip:RemoveAllChildren() end
end

function UIPanelActivityStore:RefreshItemTipsScrollViewHeight()
    ...
    local nScale = UIHelper.GetScale(self.WidgetCard) -- 考虑缩放，保证不同分辨率下拉伸后Tips长度的一致性
    local nH = UIHelper.GetHeight(self.WidgetCard) / nScale
    local nDeltaHeight = nH - (nScrollViewContentWidgetTop - nLayoutContentSpaceY * nChildenCount)
...
function UIPanelActivityStore:GeneratePervFilter(bAutoSelect)
...
function UIPanelActivityStore:RedirectFilter(scriptView, nLastChoosedIndex, nLastChoosedSubIndex)
...
function UIPanelActivityStore:IsDefaultSelected(tbSelected)
...
function UIPanelActivityStore:UpdateAllCells()
    self.bNeedRefresh = true
    self:UpdateSelectedGoodsDetails()
end

function UIPanelActivityStore:UpdateCurCells()
...
function UIPanelActivityStore:RefreshBuyBackCell()
...
-- 生成筛选器
function UIPanelActivityStore:GenerateSelector(tSelector, aFilterGoods, szSelectorName, bAutoSelect)
    -- 暂时认为自动筛选
    ...
    if szSelectorName == 'SchoolSelector' or
        ...
        for _, goods in ipairs(tResult) do
            if goods.tSelector[szSelectorName] and
                    not goods.tSelector[szSelectorName][''] then
                -- 不是通用装备
    ...
    elseif szSelectorName == 'RequireLevelSelector' or
            ...
            szSelectorName == 'ScoreSelector' then
        -- 范围值类
        -- 先取得所有取值枚举列表
...
function UIPanelActivityStore:SetSelector(tSelector, szSelectorName, key, bSelected)
...
-- 获取筛选结果
function UIPanelActivityStore:GetSelectorResult(tSelector, aFilterGoods, szIngoreSelectorName)
    local aGoods = {}
    ...
    local fnCheckEnumSelector = function(szSelectorName)
        ...
        if aSelector and szIngoreSelectorName ~= szSelectorName and aSelector.nSelected and aSelector.nSelected > 0 then
            for i = #aGoods, 1, -1 do
                ...
                if goods.tSelector[szSelectorName] and
                        goods.tSelector[szSelectorName][''] then
                    -- 通用物品 全门派装备
                    bShow = true
                else
                    for _, selector in ipairs(aSelector) do
...
function UIPanelActivityStore:ApplySelector()
    if self.nShopMode == 'SHOP' then
...
function UIPanelActivityStore:SortGoods()
    local player = GetClientPlayer()
...
function UIPanelActivityStore:ApplySearchFilter()
...
function UIPanelActivityStore:CanMutiBuy(goods)
    return CanMutiBuy(goods, self.aShopInfo)
end

function UIPanelActivityStore:ForceRefreshGoods(bGenerateFilter)
...
function UIPanelActivityStore:ForceFixQuantityController()
    if not self.scriptQuantityController then
        return
    end
...
function UIPanelActivityStore:AutoRefreshCostItem()
    if not self.nRemarkCostTick or (self.nRemarkCostTick and self.nRemarkCostTick + 5 >= GetLogicFrameCount()) then
        return
    end
...
function UIPanelActivityStore:UpdateSkin()
    local tInfo = Table_GetFullScreenShopSkin(self.nFullScreenType)
    if not tInfo then
        return
    end

    UIHelper.SetSpriteFrame(self.ImgBg, tInfo.szVKBgPath, true, true) -- 商店背景

    local tBtnInfo = Table_GetFullScreenShopBtn(tInfo.ShopCloseAnimation)
    UIHelper.SetSpriteFrame(self.ImgClose, tBtnInfo.szVKPath, true, true) -- 右上角关闭

    tBtnInfo = Table_GetFullScreenShopBtn(tInfo.ShopBuyAnimation)
    UIHelper.SetSpriteFrame(self.scriptQuantityController.ImgConfirm, tBtnInfo.szVKPath, true, true) -- Tip按钮
    UIHelper.SetSpriteFrame(self.scriptQuantityController.ImgBuy, tBtnInfo.szVKPath, true, true)
    UIHelper.SetSpriteFrame(self.scriptQuantityController.ImgPreview, tBtnInfo.szVKPath, true, true)

    tBtnInfo = Table_GetFullScreenShopBtn(tInfo.ShopLeftAnimation)
    UIHelper.SetSpriteFrame(self.scriptQuantityController.ImgDecrease, tBtnInfo.szVKPath, true, true) -- -号

    tBtnInfo = Table_GetFullScreenShopBtn(tInfo.ShopRightAnimation)
    UIHelper.SetSpriteFrame(self.scriptQuantityController.ImgAdd, tBtnInfo.szVKPath, true, true) -- +号

    tBtnInfo = Table_GetFullScreenShopBtn(tInfo.ShopStepAnimation)
    UIHelper.SetSpriteFrame(self.scriptQuantityController.ImgHandle, tBtnInfo.szVKPath, true, true) -- 圆点
end

-----------------------------------场景相关-------------------------------------

function UIPanelActivityStore:GetCurItem()
    local goods = self.aGoods and self.aGoods[self.nCurGoodsIndex]
    local hItem, bItem = ShopData.GetItemByGoods(goods)
    return hItem
end

function UIPanelActivityStore:GetCurItemInfo()
    if self.modelManager then
        return self.modelManager:GetCurrentItemInfo()
    end
end

function UIPanelActivityStore:GetCurItemType()
    return self.modelManager and self.modelManager.szType or ""
end

return UIPanelActivityStore