...
UITraversalStart = {}
UITraversalStart.bSwitch = false
function UITraversalStart.FrameUpdate()
    if not UITraversal.bSwitch then
        Timer.DelAllTimer(UITraversalStart)
        StabilityController.bFlag = true
    end
    if UITraversalStart.bSwitch then
        if UITraversal.bSwitch then
            -- 启动商城遍历
            UITraversal.Start()
            UITraversalStart.bSwitch = false
        end
...
Timer.AddCycle(UITraversalStart,1,function ()
    UITraversalStart.FrameUpdate()
end)
