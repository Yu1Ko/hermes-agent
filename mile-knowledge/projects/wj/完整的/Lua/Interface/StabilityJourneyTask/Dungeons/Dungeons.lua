...
local TaskPath
function DungeonsRunMapTask(TaskPath)
    for line in io.lines(TaskPath) do
...
function Dungeons.SplitState(nLine)
    tbTaskState = {}
    tbCustom = {}
    tbCoordinate = {}
...
local bflowGM = false
local FlowGM = BaseState:New("FlowGM")
...
-- 特殊按钮操作
local BtnOperate = BaseState:New("BtnOperate")
...
-- 关闭所有的帧函数
local function DellTimer()
    Timer.DelAllTimer(DungeonsCloseView)
...
DungeonsStart = {}
DungeonsStart.bSwitch = false
function DungeonsStart.FrameUpdate()
    if not Dungeons.bSwitch then
    ...
    if DungeonsStart.bSwitch then
        if Dungeons.bSwitch then
            Dungeons.Start()
            Dungeons.SplitState(nTaskLine)
            -- 启动对应的帧函数
            -- 弹窗
...
Timer.AddCycle(DungeonsStart,1,function ()
    DungeonsStart.FrameUpdate()
end)
