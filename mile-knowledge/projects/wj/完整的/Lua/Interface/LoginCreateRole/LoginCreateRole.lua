...
-- 登录账号 密码 服务器 大区
local szAccount=RandomString(8)          --账号随机八位数字
...
--读取tab的内容
local tbRunMapData=SearchPanel.LoadRunMapFile(SearchPanel.szInterfacePath.."LoginCreateRole/RunMap.tab",2)
list_RunMapCMD = tbRunMapData[1]
list_RunMapTime = tbRunMapData[2]
local bFlagA = true	-- 是否执行前后置命令
...
function Preconditions.FrameUpdate()
    ...
    if bFlagA and GetTickCount()-nCurrentTimeA>nNextTime*1000 then
        ...
        nNextTime=nTime
        --启动操作
        if string.find(szCmd,"LoginCreateRole_start") then
            --启动登录创角
			bstart=true
		...
		nCurrentTimeA=GetTickCount()
        nCurrentStep=nCurrentStep+1 
    end
end

Timer.AddFrameCycle(Preconditions,1,function ()
