if not SearchItemArmor then
    SearchItemArmor = {
        className = "SearchItemArmor",
        text = '【防具】',
        szPlaceHolder = '输入防具ID或名称',
...
function SearchItemArmor:ShowSubWindow(tbGMView)
    tbGMView.PanelRightView:setVisible(true)
    tbGMView.LabelExtension:setVisible(true)
    tbGMView.EditSearchRight:setPlaceHolder(SearchItemArmor.szPlaceHolder)
    ...
    UIHelper.SetString(tbGMView.LabelExtension, SearchItemArmor.text)
end

function SearchItemArmor:OnClick(tbGMView)
    if not next(SearchItemArmor.tItem) then
        OutputMessage("MSG_SYS", "第一次加载需要一点时间, 请稍等...\n")
        Timer.AddFrame(SearchItemArmor, 15, function()
            SearchItemArmor.FillAll()
    ...
    SearchItemArmor:ShowSubWindow(tbGMView)
    UIHelper.SetString(tbGMView.EditSearchRight, "")
    tbGMView.tbGMPanelRight = SearchItemArmor
    tbGMView.tbRawDataRight = SearchItemArmor.tItem
    tbGMView.tbSearchResultRight = SearchItemArmor.tItem
    UIHelper.TableView_init(tbGMView.LuaTableViewRight, #tbGMView.tbRawDataRight, PREFAB_ID.WidgetSunmon)
    UIHelper.TableView_reloadData(tbGMView.LuaTableViewRight)
end
...
if not _G.bClassic then
    tItemBaseHead = {
        Path = "\\settings\\item\\Custom_Armor.tab",--PAK需要拷贝的表
        Title =
        {
            {f = "i", t = "ID"},
            {f = "s", t = "Name"},
...
function SearchItemArmor.LoadTable(tItemBaseHead)
    SearchItemArmor.tTabBase = KG_Table.Load(tItemBaseHead.Path, tItemBaseHead.Title, TABLE_FILE_OPEN_MODE.NORMAL)
end

function SearchItemArmor.FillAll()
    SearchItemArmor.LoadTable(tItemBaseHead)
    local nCount = SearchItemArmor.tTabBase:GetRowCount()
    for i = 1, nCount do
        local tItem = SearchItemArmor.tTabBase:GetRow(i)
        ...
        table.insert(SearchItemArmor.tItem, tTemp)
    end
end

function SearchItemArmor:BtnOperate(tbData)
    SendGMCommand("player.AddItem(7 ,".. tbData.ID ..")") --这里要修改道具类型
    OutputMessage("MSG_ANNOUNCE_NORMAL", "获得防具【" ..UIHelper.GBKToUTF8(tbData.Name).. "】\n")
end

function SearchItemArmor:GetAllData(tbGMView)
    tbGMView.tbSearchResultRight = SearchItemArmor.tItem
