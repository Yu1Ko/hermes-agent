local _M = {className = "LangKeXingProgressInfoHandler"}
local self = _M

--浪客行
_M.szInfoType = TraceInfoType.LangKeXingProgress
...
function _M.RegEvent()
    Event.Reg(self, EventType.On_Update_GeneralProgressBar, function(tbInfo)
        if tbInfo.szName == "bar59" then
            self.tbPBInfo = tbInfo
            TraceInfoData.UpdateInfo(TraceInfoType.LangKeXingProgress)
            Event.Dispatch(EventType.OnTogTraceInfo, TraceInfoType.LangKeXingProgress, true)
            if not self.bFirstOpen then
                Event.Dispatch(EventType.OnSetTraceInfoPriority, TraceInfoType.LangKeXingProgress)
                self.bFirstOpen = true
            end
        end
    end)
    Event.Reg(self, EventType.On_Delete_GeneralProgressBar, function(szName)
        if szName == "bar59" then
            if self.tbPBInfo then
                self.tbPBInfo = nil
            end
            Event.Dispatch(EventType.OnTogTraceInfo, TraceInfoType.LangKeXingProgress, false)
...
function _M.OnUpdateView(script, scrollViewParent, tData)
    self.UpdateLKXProgressInfo(script, scrollViewParent)
end
...
function _M.UpdateLKXProgressInfo(script, scrollViewParent)
