...
function UIVideoPlayer:OnEnter()
...
function UIVideoPlayer:BindUIEvent()
...
function UIVideoPlayer:DelayCloseView()
    Timer.Add(self , 0.2 , function ()
        UIMgr.Close(self)
    end)
end

function UIVideoPlayer:RegEvent()
...
function UIVideoPlayer:Play(szUrl, tConfig, fnEndCall)
    self.szUrl = szUrl
    ...
    if not tConfig.bKeepSound then
...
function UIVideoPlayer:SetStoryState(bIsStory)
    self.bIsStory = bIsStory
end

function UIVideoPlayer:Close()
...
function UIVideoPlayer:WaitCloseView()
    if self.videoPlayer:isPlaying() then
        self.videoPlayer:stop()
    else
...
function UIVideoPlayer:OpenButtonHideState()
...
function UIVideoPlayer:AdjustViewScale()
...
function UIVideoPlayer:SettingSound(bEnable)
    if bEnable then
