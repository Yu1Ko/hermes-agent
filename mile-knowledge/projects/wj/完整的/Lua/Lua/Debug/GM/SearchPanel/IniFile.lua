...
--File:        IniFile.lua
--Author: 	  zouhui
--Date: 	  2011/4/3 16:28:15
--Describe:  ini文件操作
-------------------------------------------------------------------

Lib			= Lib or {};
Lib.IniFile		= Lib.IniFile or {};
local IniFile	= Lib.IniFile;
local _tbMeta	= {};
...
function IniFile:LoadFile(szFileName)
...
-- 载入数据细节
function IniFile:_LoadDetail(szFileName)
	...
	if not text then
		...
		LOG.INFO("<open file : [" .. UIHelper.GBKToUTF8(szFileName) .. "] failed>")
		return;
	end
	...
	for _, line in ipairs(strList) do
		...
		if line and "" ~= line and 1 ~= self:_IsNodes(line) then
			if 1 == self:_IsSection(line) then		-- 是一个节
			...
			elseif 1 == self:_IsKeyValue(line) then	-- 是一对key，value
...
-- 清除开始和最后的无效字符
function IniFile:_ClearInvalid(line)
	line = line or "";
	local nBegin = 1;
	local nEnd = #line;
...
-- 是否是注释
function IniFile:_IsNodes(line)
	line = line or "";
	return tbNodeMark[string.sub(line, 1, 1)] or 0;
end
-- 是否是一个节名
function IniFile:_IsSection(line)
	line = line or "";
...
-- 是否是对于的key，value
function IniFile:_IsKeyValue(line)
	line = line or "";
...
-- 获取节名
function IniFile:_GetSectionName(line)
	line = line or "";
	return string.sub(line, 2, #line - 1);
end
-- 获取对应的key，value
function IniFile:_GetKeyValue(line)
	local b, e = string.find(line, "=");
	local szKey = "";
	local szValue = "";
	if b and e then
