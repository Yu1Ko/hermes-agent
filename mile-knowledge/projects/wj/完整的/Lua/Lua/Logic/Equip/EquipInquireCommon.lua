...
local ITEM_SRC_TYPE =
{
	...
	CONVERSION = STR_OBJECT.STR_SRC_TYPE[12], -- 牌子兑换
	JUSTICE = STR_OBJECT.STR_SRC_TYPE[13], -- 侠义值
}
...
local FLITER_TYPES =
{
	...
	[ITEM_SRC_FLITER_TYPE.SHOP] = {
		...
		ITEM_SRC_TYPE.CONVERSION,
		ITEM_SRC_TYPE.JUSTICE,
		ITEM_SRC_TYPE.ACTIVITY,
	},
...
local function MatchString(szSrc, szDst)
	local nPos = string.find(szSrc, szDst, 1, true)
	if not nPos then
	   return false;
	end
...
local function JudgeSchool(tSrcSchool, DstSchool, TypeAll)
	...
	for _, szSchoolID in ipairs(tSrcSchool) do
		...
		elseif DstSchool == STR_OBJECT.STR_TYPE_ALL then
			return true
		end
	end
end
...
local function CheckSrcDescMatch(szDesc, srcDescKey)
	if szDesc and szDesc ~= "" then
		tSourceDesc = GetPaseResult(szDesc, "{", "}")
		for _, t in pairs(tSourceDesc) do
			if type(t) == "table" then
				for _, v in pairs(t) do
					if v == srcDescKey then
						return true
					end
				end
			elseif t == srcDescKey then
				return true
			end
		end
	end
...
local function GetDropRateText(fDropRate)
	...
	if szDropRate == "0.00%" then
		szDropRate = UIHelper.UTF8ToGBK(g_tStrings.STR_GUILD_LAST_ONLINE_TIME_UNKNOWN)
	end
...
local function GetDungeonDesc(tSourceDesc, tBelongMapID, szOther, szBossPrefix)
	local szUnknow = UIHelper.UTF8ToGBK(g_tStrings.STR_GUILD_LAST_ONLINE_TIME_UNKNOWN)
...
local function GetItemSourceDesc(szSourceType, tSourceDesc, tBelongMapID, szSourceForce , szPrestigeRequire)
	...
	if JudgeSourceType(szSourceType, ITEM_SRC_FLITER_TYPE.SHOP) then  --商店 shop
		if szSourceType == ITEM_SRC_TYPE.REPUTATION then
			local szOther = szSourceType..STR_OBJECT.STR_SHOP..UIHelper.UTF8ToGBK(g_tStrings.STR_COLON)
		...
		elseif szSourceType== ITEM_SRC_TYPE.CONTRIBUTE then
			local szOther = STR_OBJECT.STR_TONG_SHOP..UIHelper.UTF8ToGBK(g_tStrings.STR_COLON)
	...
	elseif JudgeSourceType(szSourceType, ITEM_SRC_FLITER_TYPE.WORLD) then  --世界
		...
		elseif szSourceType == ITEM_SRC_TYPE.OUTDOORBOSS then
			szText = szText..GetDungeonDesc(tSourceDesc, tBelongMapID, UIHelper.UTF8ToGBK(g_tStrings.HEADER_SHOW_MAP), STR_OBJECT.STR_BOSS_YEWAI)
		else
			szText = szText..GetDungeonDesc(tSourceDesc, tBelongMapID, UIHelper.UTF8ToGBK(g_tStrings.HEADER_SHOW_MAP), STR_OBJECT.STR_BOSS_YEWAI)
		end
	...
	elseif szSourceType== ITEM_SRC_TYPE.QUEST or
		...
		szText = szText .. GetFormatText(szSourceType.. UIHelper.UTF8ToGBK(g_tStrings.STR_COLON))
		if not tSourceDesc or #tSourceDesc == 0 then
			szText = szText .. GetFormatText(szUnknow)
		else
			szText = szText .. GetCommonDesc(tSourceDesc)
...
function EquipInquire_GetItemSourceDesc(tInfo, bNotNewLine)
	local szText = ""
	...
	if szText == "" then
		szText = GetFormatText(UIHelper.UTF8ToGBK(g_tStrings.STR_GUILD_LAST_ONLINE_TIME_UNKNOWN))
	end
	return szText
end
...
function EquipInquire_SearchEquip(
	...
	for i=2, nCount do
		...
		if itemInfo then
			...
			Log("item dwTabType="..tLine.dwTabType.." nItemID="..tLine.nItemID.." is not Exist!")
		end
	end
	return tEquip
end
	...
	for i = 2, nCount do
		...
		if JudgeSourceType(tLine.szSourceType, ITEM_SRC_FLITER_TYPE.SHOP) then
			...
			if szShopName == EQUIP_SEARCH_ALL then
				bChoose = true
			elseif MatchString(tLine.szSourceDesc, szShopName) and CheckSrcDescMatch(tLine.szSourceDesc, szShopName) then
				bChoose = true
			end
			if bChoose then
				local tData = FormatItemInfo(tLine)
...
function EquipInquire_PopupMenu(hBtn, text, tData)
	...
	local menu =
	{
		...
		fnAutoClose = function() return not EquipInquire.IsOpened() end,
	}
...
function EquipInquire_DungeonFirstAndSecond(nMapID, szName)
	local FindBoss = function(tSecond, key1, key2)
		local wkey1, wkey2
		for i = 0, 10, 1 do
			if i == 0 then
				wkey1 = key1
			...
			if not tSecond[wkey1] then
				return
			end

			if nMapID == tSecond[wkey1] and tSecond[wkey2] then
				for _, szBossName in ipairs(tSecond[wkey2]) do
	...
	for k, tFirst in ipairs(STR_OBJECT.DUNGEON) do
		...
		for _, tSecond in ipairs(tFirst.tDungeon) do
			...
			if FindBoss(tSecond, "nNormalID", "tNormalBoss") then
				return szFirstName, szSecondName
			elseif FindBoss(tSecond, "nHardID", "tHardBoss") then
				return szFirstName, szSecondName
			end
		end
	end
...
function EquipInquire_FormatSource(tInfo)
	local tResult = {}
	if tInfo.szSourceDesc and tInfo.szSourceDesc ~= "" then
		local szSourceDesc, szDesc = string.match(tInfo.szSourceDesc, "(.+)(%b())")
		if szSourceDesc then
...
function DungeonDrop_Search(szBossName, nMapID, szSchool, szMagicKind)
	if not szSchool then
		szSchool = STR_OBJECT.STR_TYPE_ALL
	end

	if not szMagicKind then
		szMagicKind = STR_OBJECT.STR_TYPE_ALL
	end

	local tDropList = DungeonDrop_SearchByMapAndBossName(szBossName, nMapID)
	tDropList = DungeonDrop_FromListSearch(tDropList, szSchool, szMagicKind)
	return tDropList
...
function DungeonDrop_FromListSearch(tDropList, szSchool, szMagicKind)
	...
	for _, tItem in ipairs(tDropList) do
		if  JudgeSchool(tItem[5]:split("|"), szSchool, STR_OBJECT.STR_TYPE_ALL)
		and JudgeType(tItem[6], szMagicKind, STR_OBJECT.STR_TYPE_ALL) then
