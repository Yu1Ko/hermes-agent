...
class PerfettoThread(threading.Thread):
    def __init__(self, target_func,  *args, **kwargs):
        super(PerfettoThread, self).__init__()
...
class Perfetto():
  ...
  def __init__(self, serial = None, case_name = "") -> None:
    ...
    # 开始开启服务
    self.adb('shell', '/data/local/tmp/gpu_launcher64')
  ...
  def find_adb(self):
    ...
    if self.adb_path is None:
      ...
      # sys.exit(1)
      raise Exception("Could not find a suitable adb binary in the PATH.")

  def adb(self, *args, stdin=devnull, stdout=None):
  ...
  def start(self):
    ...
    probe = self.adb('shell', probe_cmd, stdout=subprocess.PIPE)
    ...
    api_level = int(lines[0]) # pixel4 is 30
    abi = lines[1]            # pixel4 is arm64-v8a
    arch = ABI_TO_ARCH.get(abi)
    if arch is None:
      prt('Unsupported ABI: ' + abi)
      # sys.exit(1)
      raise Exception(f"Unsupported ABI: {abi}")
    shell_user = lines[2]     # pixel4 is shell
    if api_level < 29:  # 29: Android Q.
      tracebox_bin = None
      if tracebox_bin is None:
        tracebox_bin = get_perfetto_prebuilt('tracebox', arch='android-' + arch)
      perfetto_cmd = '/data/local/tmp/tracebox'
      exit_code = self.adb('push', '--sync', tracebox_bin, perfetto_cmd).wait()
      exit_code |= self.adb('shell', 'chmod 755 ' + perfetto_cmd).wait()
      if exit_code != 0:
        prt('ADB push failed', ANSI.RED)
        # sys.exit(1)
        raise Exception("ADB push failed")
    ...
    self.device_file = device_dir + fname
    sdcard_file = sdcard_dir + fname

    cmd = [perfetto_cmd, '--background', '--txt', '-o', self.device_file]
    ...
    host_dir = os.path.join(CUR_DIR, "perfetto")
    self.host_file = os.path.join(host_dir, f"{tstamp}_{self.case_name}.pftrace")
    print(host_dir)
    if not os.path.exists(host_dir):
      shutil.os.makedirs(host_dir)

    with open(self.config or os.devnull, 'rb') as f:
      print('Running ' + ' '.join(cmd))
      proc = self.adb('shell', *cmd, stdin=f, stdout=subprocess.PIPE)
      self.bg_pid = proc.communicate()[0].decode().strip()
      exit_code = proc.wait()

    if exit_code != 0:
      prt('Perfetto invocation failed', ANSI.RED)
      self.kill_all_subprocs_on_exit()
      raise Exception("Perfetto invocation failed")
  ...
  def stop(self):
    ...
    #   try:
    #     poll = self.adb('shell', 'test -d /proc/%s || exit 42' % self.bg_pid)
    #     poll_res = poll.wait()
    #     if poll_res == 42:
    ...
    #       # sys.exit(1)
    #       raise Exception("Too many unrecoverable ADB failures, bailing out")
    #     time.sleep(2)
    ...
    self.adb('shell', 'kill -%s %s' % (sig, self.bg_pid)).wait()

    prt('\n')
    prt('Pulling into %s' % self.host_file, ANSI.BOLD)
    # adb('shell', 'su', '-c', 'cp', device_file, sdcard_dir).wait()
    self.adb('pull', self.device_file, self.host_file).wait()
    self.adb('shell', 'rm -f ' + self.device_file).wait()

    # prt('\n')
    # prt('Opening the trace (%s) in the browser' % self.host_file)
    # open_trace_in_browser(self.host_file)

    self.kill_all_subprocs_on_exit()
    if os.path.exists(self.host_file):
      return {"file_path": self.host_file}
    else:
      return None

  def kill_all_subprocs_on_exit(self):
    for p in [p for p in self.procs if p.poll() is None]:
      p.kill()
...
def main():
  ...
  # 开始开启服务
  adb('shell', '/data/local/tmp/gpu_launcher64')
  ...
  if args.list_ftrace:
    adb('shell', 'su', '-c', 'cat /d/tracing/available_events | tr : /').wait()
    sys.exit(0)
  ...
  if args.out.endswith('/') or os.path.isdir(args.out):
    ...
    if host_dir == '':
      ...
      host_file = './' + host_file
  host_dir = os.path.join(CUR_DIR, "perfetto")
  host_file = os.path.join(host_dir, fname)
  print(host_dir)
  if not os.path.exists(host_dir):
    shutil.os.makedirs(host_dir)
