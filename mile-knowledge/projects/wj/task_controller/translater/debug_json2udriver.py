...
class Json2Udriver:
    ...
    def Translate(self):
        with open(file=self.file_path, mode='r', encoding='utf-8') as f:
            ...
            data_str += 'from altunityrunner import AltrunUnityDriver\n'
            data_str += 'from altunityrunner import By\n'
            data_str += 'import time\n\n'

            data_str += 'def RunTest(HOST = "127.0.0.1", PORT = 13000):\n'

            tap_count += 1

            data_str += '\t' * tap_count + 'udriver = AltrunUnityDriver(None, "", HOST, TCP_PORT=PORT, timeout=60, log_flag=True)\n\n'

            data_str += '\t' * tap_count + 'auto_test_finish_count = 0\n'
            ...
            for data_item in res:
                ...
                if(self.click and eventHandler.endswith('ClickHandler')):
                    ...
                    if len(ui_names) > 1:
                        ...
                        data_str += command
                        data_str += '\t' * tap_count + 'auto_test_finish_count += 1\n'
                    ...
                    if len(ui_names) > 1:
                        ui_path = reduce(lambda x,y:x+'//'+y, ui_names)
                        command = '\t' * tap_count + 'udriver.swipe('+str(position_x)+','+str(position_y)+','+str(end_x)+','+str(end_y)+','+self.get_time_str(time)+')\n'
                        data_str += command
                        data_str += '\t' * tap_count + 'auto_test_finish_count += 1\n'
            ...
            data_str += '\t' * tap_count + 'print("-" * 10 + "fail in " + str(auto_test_finish_count) + "-" * 10)\n'
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', help="settext mode")
    parser.add_argument('-u', help="upanddown mode")
