...
class CaseJX3SearchPanel(CaseJX3Client):
    ...
    def __init__(self):
        ...
        self.bExtraData=True #是否采集扩展数据
        self.list_strCmd = [] #跑图前需要执行的Cmd指令
        self.nExtraDataType=1 #扩展数据类似 1:SDK采集 2:插件本地采集
    ...
    def clearInfoFiles(self):
        ...
        listFilesName = ['perfeye_start', 'perfeye_stop',  'BeginRunMap', 'ExitGame','Renderdoc']
        #游戏客户端通信文件夹
        ...
        TEMP_FOLDER = f'{GetTEMPFOLDER()}{os.sep}Interface'
    ...
    def checkRecvInfoFromSearchpanel(self, fileName):
        ...
        #self.log.info("need find: {}".format(uFilepath))
        ...
        try:
            ...
            while True:
                ...
                if self.checkRecvInfoFromSearchpanel('AutoLoginPanel'):
                    if self.nExtraDataType != 1:
                        self.log.info("使用插件采集扩展数据")
                        break
            ...
            self.log.info(info)
            #避免在perfeye_save的时候等待PerfeyeStop报错
    ...
    #跑前执行cmd 比如设置帧率等
    def RunCMD(self):
        #处理荣耀70 和 iphone12默认帧率的问题
        if self.strNumVideoLevel=='2' and self.tagMachineType=='Android' and self.runMapType!="SkillRelease":
            self.nFrame=25

        if self.strNumVideoLevel=='3' and self.tagMachineType=='Ios' and self.runMapType!="SkillRelease":
            self.nFrame=25

        if 'nFrame' in self.args:
            self.nFrame=int(self.args['nFrame'])

        if self.nFrame>1 and "技能" not in self.strCaseName:
            self.list_strCmd.append(f"/cmd FrameMgr.SetFrameLimit({self.nFrame})")
    ...
    def EngineOption(self,dic_args):
        ...
        if "sdk" in dic_args:
            if not self.SocketClient:
                self.log.info("self.SocketClient is none")
                return
            ...
            if nSdk == 1:
                ...
                self.SocketClient.SetEngineOption(Enum_option("EO_debug_set_point_light_enable"), 0, True)
                self.SocketClient.SetEngineOption(Enum_option("EO_debug_set_spot_light_enable"), 0, True)
                self.SocketClient.SetEngineOption(Enum_option("EO_debug_set_point_light_effect_character_enable"), 0, True)
            elif nSdk==21:
                self.SocketClient.SetEngineOption(Enum_option("EO_debug_set_point_light_enable"), 0, True)
                self.SocketClient.SetEngineOption(Enum_option("EO_debug_set_spot_light_enable"), 0, True)
            elif nSdk==22:  # 开启
                self.SocketClient.SetEngineOption(Enum_option("EO_debug_set_apex_clothing_enable"), 0, True)
            elif nSdk==23:  # 开启风场
                self.SocketClient.SetEngineOption(Enum_option("EO_apex_local_wind_enable"), 0, True)
            self.log.info(f"EngineOption:{nSdk}")


        if "EngineStandard" in dic_args:
            nSdk = int(dic_args["EngineStandard"])
            if nSdk == 1:
                self.SocketClient.SetEngineOption(Enum_option("EO_debug_set_gameplay_model_enable"), 0, False)
                self.SocketClient.SetEngineOption(Enum_option("EO_basic_set_shadow_quality_int"), 1, 0)
    ...
    def thread_CheckTaskTimeOut(self, dicSwitch, t_parent):
        ...
        strScreenShotPath = os.path.join(GetTEMPFOLDER(),'TaskTimeOutScene.png')
        # 放入消息队列的消息内容
        strMsg = ''
        nExceptionType = 0
    ...
    def thread_CheckScreenShot(self, dicSwitch, t_parent):
        ...
        strScreenShotPath = os.path.join(GetTEMPFOLDER(), 'CheckScreenShot.png')
        nStepTime = 0.1
        nTimerRunMapEnd = 0
        nTimerScreenShot = 0
        nTimerKeepHeart = 0
        ...
        nReconnectionCnt=0
        #根据控制设备配置 判断是否需要上传截图
        deviceID=self.args['device']
        devicename = deviceID.split(":")[0].replace('.', '') if "10." in deviceID else deviceID
        strConfigDir = os.path.join(os.getcwd(), "device_config", f"{self.tagMachineType.lower()}-{devicename}.ini")
        if ini_get("Local", "UploadImg", strConfigDir) == '0':
            self.log.info(f"根据配置文件关闭截图线程 path:{strConfigDir}")
            return
        strScreenShotPath=os.path.join(os.getcwd(),f"log_file\stop_{devicename}.png")
        self.log.info(f"控制器截图位置: {strScreenShotPath}")
        while t_parent.is_alive():
            # 检测app是否启动
            if not self.clientPID:
                time.sleep(10)
            ...
            if nTimerRunMapEnd > 0.2:
                ...
                nTimerScreenShot = 0
                # 60秒检查一次截图
                #默认使用控制器截图 减少设备消耗,需要判断上一张截图是否更新,可能会出现卡在的情况
                if os.path.exists(strScreenShotPath):
                    fLastTime=os.path.getmtime(strScreenShotPath)#文件最近一次修改时间
                    fTime = time.time()#当前时间
                    if fTime-fLastTime>70: #70s还未更新图片 则自己截图
                        self.log.info("控制器70秒还未更新图片 用例自己更新图片")
    ...
    def thread_CheckAppRunStateAndCrash(self, dicSwitch, t_parent):
        ...
        strMsg=''
        nExceptionType=0
        strScreenShotPath = os.path.join(GetTEMPFOLDER(), 'CrashScene.png')
        nStepTime=0.1
        nTimerCheckCrash=0
        nTimerRunMapEnd=0
        nTimerKeepHeart=0
            ...
            if nEType == ExceptionMsg.CRASH or nEType == ExceptionMsg.FLASHBACK:
                ...
                try:
                    ...
                    subtags = '{0}|{1}|{2}|{3}|{4}|{5}'.format(self.tagVideoLevel, self.tagMachineType,
                                                               ...
                                                               date_get_szToday())
                    bRetXGameUploadData,data=self.perfeye.PerfeyeSave(subtags=subtags,BVT=False,strVersion=self.GetVersion())
                ...
                if strOS == 'Windows':
                    ...
                    strBasePath = r"/mnt/BaseShare/FileShare-181-242/liuzhu/JX3BVT/DumpSysLog"
                ...
                if not self.strClientLog:
                    ...
                    strAccountFilePath = f'{GetTEMPFOLDER()}{os.sep}{strAccountFileName}'
                    ...
                    self.strClientLog = filecontrol_getFolderLastestFile(self.CLIENT_LOG_PATH + '/' + strDate,
                                                                         GetTEMPFOLDER(), self.deviceId, self.package)
        ...
        try:
            ...
            while t_parent.is_alive():
                ...
                if elapseTime % 5 == 0:  # 每5秒输出一下信息，感知此线程还活着
                    self.log.info("KillDeath check running:" + str(elapseTime))
    ...
    def processInterface(self, dic_args):
        ...
        if 'env' in dic_args:
            ...
            # 是否使用插件采集扩展数据
            if self.nExtraDataType == 2:
                copyFolderNames = copyFolderNames + ["GetPerfData"]
        ...
        # 是否使用插件采集扩展数据
        if self.nExtraDataType == 2:
            strInterface = ini_get("Interface", self.runMapType, TEMP_FOLDER + '/Interface/SearchPanel/Interface.ini')
            strInterface = strInterface + ",GetPerfData"
            ini_set("Interface", self.runMapType, strInterface, TEMP_FOLDER + '/Interface/SearchPanel/Interface.ini')

        #PC端会拉取GM失败 因此需要判断Lua/Debug/GM文件夹中是否有相关文件
        if not self.bMobile or self.deviceId=='fdec25cf':
            filecontrol_copyFileOrFolder(SERVER_PATH+'/XGame/GM',self.CLIENT_PATH+'/mui/Lua/Debug/GM', self.deviceId, self.package)
    ...
    def processSearchPanelTab(self, dic_args):
        ...
        TEMP_FOLDER =os.path.join(GetTEMPFOLDER(),'Interface',self.runMapType)
        ...
        if 'bakeMapInfo' in dic_args:
            sChange.append(['_bakeMapInfo_', dic_args['bakeMapInfo']])
        ...
        if self.runMapType in list_runMapType:
            ...
            #根据插件类型 tab文件添加cmd指令
            if self.nExtraDataType==2:
                insert_before_keyword(tmp, tmp, "perfeye_start", "/cmd GetPerfData.PerfeyeSetTimeNode()	1	采集扩展数据")
                insert_after_keyword(tmp, tmp, "perfeye_stop", "/cmd GetPerfData.PerfeyeStop()	1	采集扩展数据")
    ...
    def check_dic_args(self, dic_args):
        ...
        if 'extraDataType' in dic_args:
            self.nExtraDataType=int(dic_args['extraDataType'])
    ...
    def task_process_data(self):
        ...
        dic_extraData = {}
        if self.nExtraDataType == 1:
            if self.SocketClient and self.bExtraData:
                dic_extraData = self.SocketClient.dic_dataList
        else:
            try:
                szSrcData=self.INTERFACE_PATH+f'/{self.runMapType}/Data.json'
                self.log.info(f"ExtraDataPath:{szSrcData}")
                szTempData=os.path.join(GetTEMPFOLDER(), 'Data.json')
                filecontrol_copyFileOrFolder(szSrcData,szTempData,self.deviceId, self.package)
        ...
        try:
            ...
            if not self.strClientLog and filecontrol_existFileOrFolder(self.CLIENT_LOG_PATH):
                # 结束游戏客户端后,获取日志到本地
                strDate = time.strftime(f"%Y_%m_%d", time.localtime())
        ...
        if self.bDumpCase:
            # 宕机用例数据不用重传
            nMaxRetransmissionCount = 1

        #是否保存perfeye报告,默认不保存
        bSavePerfeyeData=False
        if 'SavePerfeyeData'in self.args:
            bSavePerfeyeData=True

        bRetXGameUploadData,data = self.perfeye.PerfeyeSave(subtags=subtags,extraData=dic_extraData,strVersion=self.GetVersion())
        if bRetXGameUploadData:
            self.args["perfeyeReport"]=data
        ...
        if "BakingModel" in self.runMapType:
            self.log.info("烘焙贴图测试 需要拷贝数据到共享")
            #烘焙贴图测试 需要拷贝数据到共享
            strBakeDst=r"\\10.11.85.148\FileShare-181-242\FileShare\pengrongzheng\测试\烘培测试地图文件"
            strDate = date_get_szToday()
            strCaseName = self.strCaseName.replace('|', '-')
            strDst = f"{strBakeDst}\{strDate}\{self.strMachineName}\{strCaseName}"
            #strDst = sort_filePath(strDst)
            if not filecontrol_existFileOrFolder(strDst):
                filecontrol_createFolder(strDst)
            strTemp='TempFolder/BakeMap.txt'
            strSrc=f"{self.INTERFACE_PATH}/BakeMap.txt"
            if filecontrol_existFileOrFolder(strSrc,self.deviceId,self.package):
                filecontrol_copyFileOrFolder(strSrc,strTemp,self.deviceId,self.package)
                filecontrol_copyFileOrFolder(strTemp,strDst,self.deviceId, self.package)
            else:
                raise Exception(f"未发现该文件:{strSrc}")
    ...
    def copyEnvToClient(self):
        ...
        # XGame拷贝Interface文件夹
        filecontrol_copyFileOrFolder(os.path.join(TEMP_FOLDER, 'Interface'), self.INTERFACE_PATH, self.deviceId,self.package)
        #临时挂包外打开商城
        ...
        #外装优化挂包外测试
        if 'optimize' in self.args:
            filecontrol_copyFileOrFolder(SERVER_PATH + '/XGame/临时测试/model_faces_optimization.json',self.CLIENT_PATH + '/represent/common_mobile/model_faces_optimization.json', self.deviceId,self.package)
        else:
            if filecontrol_existFileOrFolder(self.CLIENT_PATH + '/represent/common_mobile/model_faces_optimization.json', self.deviceId,self.package):
                #filecontrol_deleteFileOrFolder(self.CLIENT_PATH + '/represent/common_mobile/model_faces_optimization.json', self.deviceId,self.package)
                pass
        #subset
        if 'subset' in self.args and self.args['subset']:
            filecontrol_copyFileOrFolder(SERVER_PATH + '\subset测试\拆分开\config\GpuSwitchOptionTab.tab',self.CLIENT_PATH + '/config/GpuSwitchOptionTab.tab',self.deviceId, self.package)
        else:
            if filecontrol_existFileOrFolder(self.CLIENT_PATH + '/config/GpuSwitchOptionTab.tab', self.deviceId,self.package):
                #filecontrol_deleteFileOrFolder(self.CLIENT_PATH + '/config/GpuSwitchOptionTab.tab', self.deviceId,self.package)
                pass

        #临时挂GC.lua文件
        if date_get_szToday()!="2025-07-21":
            #filecontrol_copyFileOrFolder(SERVER_PATH + '/XGame/GC.lua',self.CLIENT_PATH + '/mui/Lua/Misc/GC.lua', self.deviceId,self.package)
            #filecontrol_deleteFileOrFolder(self.CLIENT_PATH + '/mui/Lua/Misc/GC.lua',self.deviceId, self.package)
            pass
        ##临时包外优化
        if "阆风悬城" in self.strCaseName:
            #filecontrol_copyFileOrFolder(SERVER_PATH + '/XGame/Def.lua', self.CLIENT_PATH + '/mui/Lua/Def/Def.lua',self.deviceId, self.package)
            filecontrol_deleteFileOrFolder(self.CLIENT_PATH + '/mui/Lua/Def',self.deviceId, self.package)
    ...
    def run_local(self, dic_args):
        ...
        self.processWindows()
        self.process_extra_config(dic_args)
        ...
        #     self.log.info ('wait for PerfReporterX86 closed')
        #     time.sleep(1)

    #公共接口
    def process_extra_config(self, dic_args):
        '''NPC柔体风场测试'''
        if 'EnableNpcFlexibleWind' in dic_args:
            EnableNpcFlexibleWindfile = r"\\10.11.85.148\FileShare\xiejieshi\RunTab\NPC柔体风场的测试\EnableNpcFlexibleWind\data"
            DisableNpcFlexibleWindfile = r"\\10.11.85.148\FileShare\xiejieshi\RunTab\NPC柔体风场的测试\DisableNpcFlexibleWind\data"
            if dic_args['EnableNpcFlexibleWind']:
                self.log.info('NPC柔体风场测试 开启柔体风场')
                filecontrol_copyFileOrFolder(EnableNpcFlexibleWindfile, os.path.join(GetTEMPFOLDER(), 'data'))
            else:
                self.log.info('NPC柔体风场测试 关闭柔体风场')
                filecontrol_copyFileOrFolder(DisableNpcFlexibleWindfile, os.path.join(GetTEMPFOLDER(), 'data'))

            self.log.info('拷贝包外配置文件到客户端')
            filecontrol_copyFileOrFolder(os.path.join(GetTEMPFOLDER(), 'data'), self.CLIENT_PATH + r'\data', self.deviceId, self.package)
        #self.CLIENT_PATH
        #filecontrol_copyFileOrFolder(os.path.join(GetTEMPFOLDER(), 'Interface'), self.INTERFACE_PATH, self.deviceId,self.package)
        pass
