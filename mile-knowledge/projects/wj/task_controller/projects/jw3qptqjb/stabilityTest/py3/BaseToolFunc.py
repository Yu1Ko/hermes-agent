...
if strOS == 'Windows':
    ...
    STRSEPARATOR = '/'
    SERVER_PATH = '/mnt/windows-share/liuzhu/JX3BVT'
    SERVER_DUMPRECORD = "/mnt/windows-share/DumpAnalyse"
    SCREENSHOT_PATH = "/mnt/windows-share/luoyan2/screenshot3"
elif strOS == 'Darwin':
    STRSEPARATOR = '/'
...
def tidevice_install_ipa(strPackagepath, deviceID=None):
    ...
    print(str(ret_error, encoding='gbk'))
...
def copyFileOrFolder_android(src, dst, deviceID=None):
    ...
    for file in os.listdir(src):
        copyFileOrFolder_android(src + os.sep + file, dst + '/' + file, deviceID)


def copyFileOrFolder_ios(src, dst, bundleID, deviceID=None):
    print(src + ' :   ' + dst)
    if '.' in src:
    ...
    for file in os.listdir(src):
        copyFileOrFolder_ios(src + os.sep + file, dst + '/' + file, bundleID, deviceID)
...
def filecontrol_copyFileOrFolder(src, dst, deviceID=None, bundleID=None):
    ...
    if strOS == "Windows":
        ...
        if '/Documents' in dst:
            ...
            copyFileOrFolder_ios(src, dst, bundleID, deviceID)
            return True
...
class TDRRobot:
    ...
    def mallocRobot(self,robotNum):
        r = requests.get(self.apiUrl+"mallocRobot", params={"robotNum": robotNum})
        r.raise_for_status()
        data = r.json()["data"]
