...
class Android_IOS(object):
    ...
    def install_app(self,strPackagepath,bFlag=True):
        ...
        if self.platform == 'android':
            ...
            try:
                ...
                res = str(res, encoding='utf8')
        if 'success' in res.lower() or 'complete' in res.lower():
    ...
    def get_battery(self):
        if self.platform == 'android':
            cmd = f"adb -s {self.deviceId} shell dumpsys battery |{self.system} level"
            pi = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            res = pi.stdout.read()
            try:
                res = str(res, encoding='gbk')
    ...
    def start_app(self,packageName=None):
        ...
        if self.platform == 'android':
            ...
            cmd = f"tidevice -u {self.deviceId} launch {packageName}"
            pi = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE)
            res = pi.stdout.read()
            try:
                res = str(res, encoding='gbk')
                ...
                res = str(res, encoding='utf8')
            self.log.info(f"app_start_result:{res}")
            return res
    ...
    def unlock(self):
        if self.platform == 'android':
            os.system(f'adb -s {self.self.deviceId}  shell input keyevent 224')  # 点亮屏幕
            time.sleep(1)
            with  os.popen(f'adb -s {self.self.deviceId} shell "dumpsys deviceidle | grep mScreenOn"') as p:
                res = p.read()  # 获取手机状态
            if 'mScreenOn=false' in res:  # 当手机为息屏状态时候
                os.system(f'adb -s {self.self.deviceId}  shell input keyevent 26')
            wh = os.popen(f"adb -s {self.self.deviceId}  shell wm size").read()
            size = wh.replace(" ", "").replace('\n', '').split(":")[1].split("x")
            size[0] = re.sub('[^0-9]', '', size[0])
            size[1] = re.sub('[^0-9]', '', size[1])
            width, height = int(size[0]), int(size[1])
            os.system(f'adb -s {self.self.deviceId} shell input swipe {width / 2} {height * 0.8} {width / 2} {height * 0.4}')
            time.sleep(3)
    ...
    def lock(self):
        ...
        if self.platform == 'android':
            os.system(f'adb -s {self.self.deviceId}  shell input keyevent 223')  # 熄灭屏幕
            time.sleep(1.50)
            with  os.popen(f'adb -s {self.self.deviceId} shell "dumpsys deviceidle | grep mScreenOn"') as p:
                res = p.read()  # 获取手机状态
            ...
            try:
                ...
                print(f"{self.self.deviceId}锁屏失败")
    ...
    def click(self, w, h):
        if self.platform == 'ios':
            ...
            os.popen(f"adb -s  {self.self.deviceId} shell input tap {height} {width}")
