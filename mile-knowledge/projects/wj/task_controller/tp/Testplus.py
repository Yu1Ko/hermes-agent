...
class TPTClient(object):
    ...
    def launch(self, launch_args):
        ...
        if self._python_exe_path:
            ...
            print("[TPTClient] subprocess cmd %s, cwd %s, time: %s" % (str(cmd), self._perfeye_path,
                                                                       datetime.datetime.fromtimestamp(
                                                                           int(time.time())).strftime(
                                                                           '%Y_%m_%d_%H%M%S')))
            try:
                with log_lock:
                    self._process = subprocess.Popen(cmd, cwd=self._perfeye_path, stdout=subprocess.PIPE,
            ...
            print("[TPTClient] subprocess cmd %s, cwd %s, time: %s" % (
            str(cmd), cwd, datetime.datetime.fromtimestamp(int(time.time())).strftime('%Y_%m_%d_%H%M%S')))
            try:
    ...
    def send_message_no_recv(self, method, params):
        ...
        for times in range(3):
            ...
            try:
                ...
                print("[TPTClient] #%d --> SEND message, method=%s, params=%s, retry=%d" % (
                self._message_id, str(method), str(params), retry_count))
                websocket.send(msg)
                time.sleep(1)
    ...
    def send_message(self, method, params, timeout=120):
        """
        Send message to tpt client
        :param method: rpc method name
        :param params: rpc method params
        ...
        while True:
            ...
            try:
                ...
                if response:
                    result = json.loads(response)
                    print("[TPTClient] #%d <-- RECV message, method=%s, response=%s" % (
                    result['id'], str(method), str(response)))
                print("disconnect from TPT client...")
                if result:
                    break
            except SystemExit as errexit:
...
class Testplus(object):
    ...
    def kill(self):
        try:
            #self._tpt_client.send_message("testplus.kill", {})
    ...
    def connect(self, serial, package_name, configs={}, pid=None):
        try:
            responsecon = self._tpt_client.send_message("testplus.connect",
                                                        {"serial": serial, "configs": configs})  # 连接设备
        except SystemExit as errexit:
            self.kill()
            raise errexit
        except BaseException as error:
            print('error', error)
            return False, "send_connect exception", None

        # 移动端在connect(app启动前)阶段 获取appinfo
        # PC端的serial必须为localhost
        if 'localhost' in serial:
            return True, responsecon['result']['msg'],None
        else:
            bRes,ret=self.get_appInfo(serial,package_name)
            return bRes,ret,self.app_info
    ...
    def save(self, serial, case_name, report_file_path=None, need_upload=True, need_save=False, appKey=None,
             scenes=None, picture_quality=None, do_upload=None, version=None, extra_data=None, timeout=600,
             parameters=None, Android_IOS=None):
        '''
        if "perfeye" in parameters['performance'] and "need_extra_data" in parameters['performance']['perfeye']:
            if parameters['performance']['perfeye']['need_extra_data'] == 1:
                extra_data = self.get_perfeye_extra_data(Android_IOS)'''
        try:
    ...
    def get_perfeye_extra_data(self, Android_IOS):
        if Android_IOS.platform == 'android':
            extra_data_Path = f"/sdcard/UE4Game/{Android_IOS.package_info['projectName']}/{Android_IOS.package_info['projectName']}/Saved/Profiling/extdata.json"
            command = f"adb -s {Android_IOS.devices} shell cat {extra_data_Path}"
        elif Android_IOS.platform == 'ios':
            extra_data_Path = f"/Documents/{Android_IOS.package_info['projectName']}/Saved/Profiling/extdata.json"
            command = f"tidevice -u {Android_IOS.devices} fsync cat {extra_data_Path}"
        try:
            process = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            out, err = process.communicate()

            if process.returncode == 0:
                json_data = out.decode('utf-8').strip()
                extra_data = json.loads(json_data)
            else:
                print(f"执行ADB命令获取{extra_data_Path}失败: {err.decode('utf-8')}")
                extra_data = None
