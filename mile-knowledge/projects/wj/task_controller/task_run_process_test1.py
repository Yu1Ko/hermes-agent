...
def get_module(paths, project_name, project_file_lock, module_path_ex):
    ...
    with project_file_lock:
        ...
        module_path = os.path.join(module_path, module_path_ex)
...
#linux------------------------------------------------
    ...
    def clean_up(self):
        ...
        try:
            ...
            if "perfeye" in task_data.keys() and task_data["perfeye"] != -1:
                logger.info(os_popen(f'ps -ef |grep miniperf'))
            ...
            if "platform" in task_data.keys():
                ...
                adio = ad_ios.Android_IOS(self.perfeyedevice_s, platform, package_url, package_info, self.project_file_lock, self.download_lock,port,temp_task_parameters)
    ...
    def do_run(self):
        ...
        try:
            ...
            if not os.path.exists("log_file"):
                os.mkdir("log_file")
            self.fzhu.add_output(threading.current_thread().ident,open(f'log_file/task{self.task_running_id}_{devices}.txt',"a+"))

            self.bot = pyfeishu.FeiShutalkChatbot(eval(self.feishu_token))
            ...
            if self.task_parameters is not None:
                try:
                    ...
                    if judge:
                        ...
                        # 自动化覆盖包体信息（使用手机上经过特殊渠道已经装好的包）
                        if "overwrite_package_info" in temp_task_parameters:
                            self.package_info = json.dumps(temp_task_parameters["overwrite_package_info"])
                except Exception as e:
                    print(f"JSON parsing error: {e}")
            self.Android_IOS = AdIo(self.device_s, self.platform, self.package_url,self.package_info,self.project_file_lock,self.download_lock,self.port,temp_task_parameters)
            ...
            self.LogMan=Logmanage(self.server_url,self.device_s,self.device_id,self.task_running_id,self.Android_IOS,self.device_ip, self.bug_log_ios_pid,self.device_name)
    ...
    def prepare_app(self):
        ...
        # 判断此次运行是否需要注入ini文件
        if "RebuildIni" in self.parameters.keys() and self.parameters["RebuildIni"] == "jxsjorigin":
            # 导入的ini文件名
            ...
            if bSucceed:
                ...
                try:
                    ...
                    if "ios" in self.platform and self.project_id=="jx1pocket":
                        self.Android_IOS.click(0.58,0.75)
                    if self.project_id=="abyss":
                        if "android" in self.platform:
                            ...
                            self.Android_IOS.WDA_U2.alert.click_exists("仅限无线局域网")
                            time.sleep(2)
                            self.Android_IOS.click(0.6,0.78)
                            time.sleep(2)
                            self.Android_IOS.WDA_U2.alert.click_exists("允许跟踪")
                            time.sleep(2)
                            self.Android_IOS.WDA_U2.alert.click_exists("仅限无线局域网")
                            time.sleep(2)
                            self.Android_IOS.WDA_U2.alert.click_exists("允许")
                            time.sleep(2)
                            self.Android_IOS.WDA_U2.alert.click_exists("允许跟踪")
                    # 初始化 u3driver
                    ...
                    if "screenshottime" in self.task_parameters.keys():
                        capture=CapTure(self, self.parameters,self.task_data,self.Android_IOS,self.perfeyedevice_s,self.device_ip,self.bot,self.device_name,self.project_id,self.project_file_lock,self.task_running_id,case["name"],self.task_parameters,int(self.task_parameters["screenshottime"]),case=case)
                    else:
                        capture=CapTure(self, self.parameters,self.task_data,self.Android_IOS,self.perfeyedevice_s,self.device_ip,self.bot,self.device_name,self.project_id,self.project_file_lock,self.task_running_id,case["name"],self.task_parameters,case=case)
                    print("capture对象初始化成功")
                    self.performance=capture.start_capture(udriver,self.Android_IOS.package)
                    if "toreversion" in self.task_parameters.keys() and self.task_parameters["toreversion"]=="true":
                        self.Android_IOS.package_info["versionName"]=self.getgamereversion(u3driver,udriver,self.Android_IOS.package_info["versionName"])
                        print("热更新版本号:",self.Android_IOS.package_info["versionName"])
                        if self.project_id == 'jxj3': #剑侠世界3会热更新版本 需要再发送一遍
                            self.bot.send_text(f"{self.device_name} 热更新版本号: {self.Android_IOS.versionName}")
                    Throw_Advice().set_gameversion(self.Android_IOS.versionName) #初始化机器人版本信息
                    ...
                    for before_run in before_runs:
                        try:
                            ...
                            # self.bot.send_text(traceback.format_exc())
                            try:
                                if not self.Android_IOS.FindRunIPA_APK(self.Android_IOS.package):#运行前置任务失败时的闪退检测
                                    self.bot.send_text(f"{self.device_name}-{self.device_s}- {case['name']} 前置条件运行中闪退")
                            except:
                                pass
                    ...
                    if self.project_id == 'jxsj4':
                        udriver.custom_interface("SetTargetFrameRate", 160)  # 设置不限帧率

                    #开始采集
                    capture.all_run_capture()
                    self.case_status.set_capture(capture)
                    if capture.chance_channel("UboxCustom"):
                        self.parameters["UboxCustom"]=ubox_custom(self.performance["ProfCustom"],case['name'],self.bot,self.device_s,self.Android_IOS.package_info["packageName"],self.Android_IOS.package_info["versionName"])
                        self.parameters["PerfeyeCustom"]=capture
                        print(f"任务参数：{self.parameters}")
                    
                    if self.project_id != "yizhiban" and self.project_id != "jw3qptqjb":  # 剑三的案例参数不一样
                        case_excute_thread = CaseRunThread(target=case_run.AutoRun, args=(udriver, self.parameters), daemon=True)
                    else:
                        perfeye = None
                        if "perfeye" in capture.performance:
                            perfeye = capture.performance["perfeye"]
                        case_excute_thread = CaseRunThread(target=case_run.AutoRun, args=(self.Android_IOS.WDA_U2, perfeye, self.LogMan.add_custom_log_file, self.parameters), daemon=True)
                    ...
                    if "performance" in self.parameters.keys():
                        collect_type = self.parameters["performance"]
                        if "memoryProfiler" in collect_type and "upload_end" in collect_type['memoryProfiler']:
                            res = capture.memory_capture(device_model=self.device_model,case=case,udriver=udriver)
                            if res and self.project_id != 'jxsj4': #剑侠世界4目前内存快照不需要上传minio
                                self.LogMan.add_extend_file(save_file=res, save=False)
                    time.sleep(5)
                    if capture.chance_channel("UboxCustom"):
                        self.report_data.update(self.parameters["UboxCustom"].capture_ubox_stop())
    ...
    def init_parameters(self, case):
        ...
        #将device_parameters存储进parameters字典中
...
class CaseStatus(object):
    ...
    def __init__(self, process_call_func,task_running_id: int, task_parameters:dict, case_id:int, device_id:int, bot: pyfeishu.FeiShutalkChatbot, execute_time_out, device_s,fzhu,task_running_status_id, build_case_id:int):
