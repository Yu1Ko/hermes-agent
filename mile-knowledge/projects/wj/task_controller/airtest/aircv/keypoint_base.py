...
def print_run_time(func):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        print("[INFO] %s() run time is %.2f s." % (func.__name__, time.time() - start_time))
        return ret
...
class KeypointMatching(object):
    ...
    def __init__(self, im_search, im_source, threshold=0.8, rgb=True):
        ...
        self.im_source = im_source#源图片
        self.im_search = im_search#目标图片
        self.threshold = threshold#阈值
        self.rgb = rgb#cv的template类默认是False的
    ...
    def find_best_result(self):
        ...
        print("[INFO] [%s] threshold=%s, result=%s" % (self.METHOD_NAME, self.threshold, best_match))
