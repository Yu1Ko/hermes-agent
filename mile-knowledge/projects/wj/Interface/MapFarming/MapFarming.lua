...
function MapFarming.Process()
    ...
    local strData = MapFarming.RecordData()
    ...
    if MapFarming.GridPointData[szIndex] then
        -- 同格已有数据，比较 Ms，只留更差的那个
        local szPrevKey = MapFarming.GridPointData[szIndex]
        local prevEntries = MapFarming.Data.performanceData[szPrevKey]
        if prevEntries and #prevEntries > 0 then
