...
local tbRunMapData=SearchPanel.LoadRunMapFile(SearchPanel.szCurrentInterfacePath.."ArenaPvP/ArenaTask.tab",6)
local list_RunMapCMD = {}                       -- CMD文件
local list_RunMapTime = {}                      -- 文件时间
ArenaPvP.list_RunMapData={}     --跑图点数据
ArenaPvP.bSwitch = true
...
ArenaPvPStart = {}
ArenaPvPStart.bSwitch = false
function ArenaPvPStart.FrameUpdate()
    if not ArenaPvP.bSwitch then
        Timer.DelAllTimer(ArenaPvPStart)
        StabilityController.bFlag = true
    end
    if ArenaPvPStart.bSwitch then
        if ArenaPvP.bSwitch then
            ArenaPvP.Start()
            ArenaPvPStart.bSwitch = false
        end
    end
...
Timer.AddCycle(ArenaPvPStart,1,function ()
    ArenaPvPStart.FrameUpdate()
end)
