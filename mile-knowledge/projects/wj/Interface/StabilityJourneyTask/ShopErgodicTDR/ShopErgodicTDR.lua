...
ShopErgodic.bSwitch = true
local ShopTitle = {}
local pCurrentTime = 0
local nNextTime=tonumber(30)
local nCurrentStep=1
local bFlag = true
local szFilePath=SearchPanel.szInterfacePath.."StabilityJourneyTask/ShopErgodicTDR/ShopMation.ini"
...
-- 摄像机旋转一周
local nX = 950
local nCameraStratTime=0 
local bCameraA =  false
-- 摄像机旋转
function CameraRotate.FrameUpdate()
    if not bCameraA then
        nCameraStratTime = GetTickCount()
        bCameraA =  true
        return 
    end
    if GetTickCount()-nCameraStratTime>nCameraTime*1000 then
        -- 有地方没有模型
        if UITouchHelper._model ~=nil then
            UITouchHelper._model:SetYaw(nX)
            if nX == -350 then
                -- 关闭体型遍历帧函数
...
function ShopHomeGoodsTitle.FrameUpdate()
    ...
    if not bShopHomeTitle then
        ...
        if nShopHomeTitleLine ~= 1 then
            UIMgr.GetViewScript(VIEW_ID.PanelSkillNew):OnSkillReplace(1, 1)
...
function ShopErgodic.Start()
    -- 启动商城遍历
    Timer.AddFrameCycle(ShopErgodicStart,1,function ()
...
ShopErgodicStart = {}
ShopErgodicStart.bSwitch = false
function ShopErgodicStart.FrameUpdate()
    if not ShopErgodic.bSwitch then
        -- 关闭帧函数
        Timer.DelAllTimer(ShopErgodicStart)
        Timer.DelAllTimer(ClsoeTeach)
        StabilityController.bFlag = true
    end
    if ShopErgodicStart.bSwitch then
        if ShopErgodic.bSwitch then
            ShopErgodic.Start()
...
Timer.AddCycle(ShopErgodicStart,1,function ()
    ShopErgodicStart.FrameUpdate()
end)
