...
if nRandomAccount == 1 then
	AutoLogin.SetAutoLoginInfo(nil,RandomString(9),nil,nil)
end
...
-- TDR包体资源
local tbTDRPack = {
	7,4,800006,800108,800580,800455,800606,800586,800668
}
...
function tbPakResource.ParkStartbase()
    ...
    if nState == DOWNLOAD_OBJECT_STATE.DOWNLOADED then
		-- 关闭资源下载面板
        if tbPakResource.ParkCorePack() then
            return true
        end
    end
...
function tbPakResource.FrameUpdate()
    if tbPakResource.base==1 then
        if tbPakResource.ParkStartbase() then
...
-- 根据步骤来执行
local EnterBuildFace = {}
EnterBuildFace.nRoletype =0
EnterBuildFace.nSchooltype =0
EnterBuildFace.nCreateRoleStartTime=0 
EnterBuildFace.nCreateRoleNextTime = 5
EnterBuildFace.nLine = 1
EnterBuildFace.VIEW_ID={
	VIEW_ID.PanelBuildFace,
	VIEW_ID.PanelBuildFace_Step2,
	VIEW_ID.PanelCreateName_Login,
}
function EnterBuildFace.FrameUpdate()
	if GetTickCount() - EnterBuildFace.nCreateRoleStartTime > EnterBuildFace.nCreateRoleNextTime*1000 then
		if not UIMgr.IsViewOpened(EnterBuildFace.VIEW_ID[EnterBuildFace.nLine]) then
			if EnterBuildFace.nLine == 1 then
				UINodeControl.BtnTriggerByLable("BtnConfirm","下一步")
			elseif EnterBuildFace.nLine == 2 then
				UINodeControl.BtnTriggerByLable("BtnNext","下一步")
			elseif EnterBuildFace.nLine == 3 then
				UINodeControl.BtnTrigger("BtnNext","WidgetAnchorRightButtom")
			end
		else
			if EnterBuildFace.nLine == 3 then
				Timer.AddCycle(InputName,1,function ()
					InputName.FrameUpdate(EnterBuildFace.nRoletype,EnterBuildFace.nSchooltype)
				end)
				Timer.DelAllTimer(EnterBuildFace)
				return
			end
			EnterBuildFace.nLine = EnterBuildFace.nLine + 1
		end
		EnterBuildFace.nCreateRoleStartTime = GetTickCount()
	end
end

-- 进入门派后的操作
function EnterBuildFace.Start()
	Timer.AddFrameCycle(EnterBuildFace,1,function ()
		EnterBuildFace.FrameUpdate()
	end)
end
...
function LoginRoleOnEnter(szPrevStep)
	...
	Timer.Add(AutoLogin,nStepTime,function ()
		...
		Timer.Add(AutoLogin,nStepTime*2,function ()
			...
			EnterBuildFace.nRoletype =nRoletype
			EnterBuildFace.nSchooltype =nSchooltype
			EnterBuildFace.Start()
		end)
