...
**⚠️ DDG HTML 端点**：`html.duckduckgo.com` 从服务器 IP 直接 curl 大概率触发 CAPTCHA（"Unfortunately, bots use DuckDuckGo too"），不要用。中文搜索直接用 Edge CDP 浏览器打开 DuckDuckGo 或 itch.io 等目标站点。
