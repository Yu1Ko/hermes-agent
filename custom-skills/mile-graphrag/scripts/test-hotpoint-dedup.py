    ...
    for key, entries in list(perf.items()):
        ...
        for i, entry in enumerate(entries):
            ...
            try:
                ...
                continue
            if ms > best_ms:
                best_ms, best_idx = ms, i
        perf[key] = [entries[best_idx]]
    return data

def main():
    if len(sys.argv) < 2:
        print("用法: python3 test_dedup.py Data.json"); return
    ...
    total_orig = total_dedup = 0
    for key in sorted(perf_orig.keys()):
        no, nd = len(perf_orig[key]), len(perf_dedup.get(key, []))
        total_orig += no; total_dedup += nd
        if no != nd:
            tag = " <- 已去重" if nd == 1 else ""
            print(f"  {key}: {no}条 -> {nd}条{tag}")
    print(f"\n去重前总条目: {total_orig}\n去重后总条目: {total_dedup}\n删除了 {total_orig - total_dedup} 条")
